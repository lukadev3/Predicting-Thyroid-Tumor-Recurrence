...
We have the same model. Now let's try more complex model, Random Forest Classifier.
...
cat("Test F1-Score:", test_f1, "\n")
```
We got a better test score, but slightly worse train score. So, let's do recap and evaluate our models.

### Model Evaluation

#### 1. Basic Decision Tree

The basic Decision Tree model was trained without cross-validation, using the training data. This model achieved decent results, but there are some limitations in accuracy on the test set.

- **Training Accuracy**: 93.94%
- **Test Accuracy**: 93.18%
- **Training Precision**: 92.73%
- **Test Precision**: 91.82%
- **Training Recall**: 95.03%
- **Test Recall**: 94.39%

The model achieved a high accuracy of 93.94% on the training set and 93.18% on the test set, which shows that it performs well. Precision and recall are also good, indicating that the model is able to make accurate predictions without many false positives or false negatives. However, there is a slight difference in performance between the training and test sets, which may suggest mild overfitting. Overfitting occurs when the model becomes too complex and performs well on training data but fails to generalize to unseen data.

#### 2. Decision Tree with 10-Fold Cross-Validation

The 10-fold cross-validation approach provides a more reliable estimate of model performance by reducing the possibility of overfitting. This model performed similarly to the basic Decision Tree, but with small improvements in F1-Score.
...
Using cross-validation helped stabilize the model's performance, making it more robust. However, the results still did not significantly differ from the basic Decision Tree model. The model continues to have high precision and recall, but perhaps more complex models could be considered to achieve even better results.

#### 3. Random Forest

The Random Forest model is a more complex model compared to the basic Decision Tree. It uses an ensemble of trees and aggregates their predictions. As a result, Random Forest often demonstrates better generalization performance.

- **Training Accuracy**: 96.97%
- **Test Accuracy**: 94.55%
- **Training Precision**: 95.76%
- **Test Precision**: 94.55%
- **Training Recall**: 98.14%
- **Test Recall**: 94.55%
- **Training F1-Score**: 96.93%
- **Test F1-Score**: 94.55%

The Random Forest model outperforms the Decision Tree models, with a much higher training accuracy of 96.97% and test accuracy of 94.55%. The precision and recall are also significantly better, demonstrating that the model can both accurately classify the data and minimize false positives. Furthermore, the model appears to generalize better, as seen in the similarity between its training and test performance. This suggests that Random Forest is more robust and less prone to overfitting.

#### 4. Random Forest with 10-Fold Cross-Validation

Adding 10-fold cross-validation to the Random Forest model further improves the estimation of the model's performance, helping to reduce the risk of overfitting.

- **Training Accuracy**: 96.06%
- **Test Accuracy**: 95.45%
- **Training Precision**: 92.73%
- **Test Precision**: 92.73%
- **Training Recall**: 99.35%
- **Test Recall**: 98.08%
- **Training F1-Score**: 95.92%
- **Test F1-Score**: 95.33%

With 10-fold cross-validation, the Random Forest model showed even more stable results. The model achieved an accuracy of 95.45% on the test set, with high precision and recall scores. This means that the model can generalize well to unseen data while minimizing false positives. The F1-score remained high for both training and test sets, indicating a good balance between precision and recall. The slight increase in test accuracy compared to the basic Random Forest model further confirms that cross-validation helped improve the model's performance.

### Conclusion

- **Decision Tree** models are simple and efficient but can be prone to overfitting, especially without cross-validation. However, they still achieve good performance on smaller datasets.
- **Random Forest** models perform significantly better in terms of accuracy, precision, and recall, as they use multiple trees to make predictions. This model is more robust and generalizes better to test data.
- **Cross-validation** improves the performance of both the Decision Tree and Random Forest models by providing a more stable and reliable evaluation, reducing the risk of overfitting.

Based on the evaluation results, the **Random Forest** model with 10-fold cross-validation appears to be the best choice for this task, offering the best balance between accuracy, generalization, and the minimization of overfitting.
